...
void loop()
{
	wait = random(60, 80);  //set a random wait period
	randpix = random(0, numpix + 1); //choose a random number of pixels
	color = random(0, 2); //Pick either yellow or orange
	for (int i = 0; i < numpix / 3; i = i++) //so it leaves a certain number of yellow pixels on (number of pixels/3)
	{
	...
	{
		flickYellow(strip.Color(255, 130, 0), wait, randpix); // call flickYellow and pass it the yellow color values (change values to change color), and wait time and random pixel count
	}
}  //end main loop
